...
int one_half[4]={0,3,7,5};
int other_half[4]={1,2,6,4};
...
# pragma omp parallel for private(k),firstprivate(j),shared(octree)
            ...
            for(k=0;k<4;k++){       
               ...
               // printf("aux=%x\t",&aux);
                ...
                if((octree->root->children[one_half[k]]!=NULL) && (octree->root->children[one_half[k]]->children[j]!=NULL)){
                    aux = octree->root->children[one_half[k]]->children[j];
                 //   printf("root->%d->%d\n",k,j);
                    aux_location[1]= one_half[k];
                    aux_location[2]= j;
                    mk_neighborhood(octree, aux, aux_location);
                }
            }

# pragma omp parallel for firstprivate(j),shared(octree)
            for(k=0;k<4;k++){       
                char aux_location[octree->max_depth];
                octree_node * aux=NULL; 
               // printf("aux=%x\t",&aux);
                aux_location[0]=octree->root->location;
                if((octree->root->children[other_half[k]]!=NULL) && (octree->root->children[other_half[k]]->children[j]!=NULL)){
                    aux = octree->root->children[other_half[k]]->children[j];
                 //   printf("root->%d->%d\n",k,j);
                    aux_location[1]= other_half[k];
                    aux_location[2]= j;
                    mk_neighborhood(octree, aux, aux_location);
                }
            }
